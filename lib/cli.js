...
const defaultConfigFile = path.resolve(__dirname, "../config/config.default.json")
...
`, {
    ...
    flags: {
      ...
      config: {
        type: "string",
        alias: "c",
        default: process.env.CONFIG_FILE || defaultConfigFile,
      },
    },
  })
  ...
  const CONFIG_FILE = path.resolve(flags.config, "./")
  ...
  for (let file of input) {
    await (file === "-" ? getStdin() : fs.readFile(file))
