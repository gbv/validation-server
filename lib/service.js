...
async function addValidators(format, service) {
  ...
  } else if (!schemas.length && !format.validateAll) {
    const { parse } = format
    if (parse) {
  ...
  // TODO: if .validateAll is given explicitly: what to do with schemas?
  ...
  format.valid = data => format.validateAll(data)
    .then(([errors]) => {
      if (errors === true) {
        return data
      } else {
...
const createService = async config => {
  ...
  const schemaFiles = {
    ...
    "https://www.w3.org/2009/XMLSchema/XMLSchema.xsd": "./schemas/XMLSchema1.1.xsd",
  }
  for (const [uri, file] of Object.entries(schemaFiles)) {
    await cache.put(uri, fs.readFileSync(path.resolve(__dirname, file)))
  }

  cache.files().then(files => config.logger.info(`cachePath ${cachePath} contains ${files.length} entries`))
