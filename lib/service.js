...
async function addValidators(format, service) {
  ...
  if (!schemas.length && !format.validate) {
    ...
    if (parse) {
      ...
      warn(`format ${id} has no schemas for validation`)
      return
    }
  }
  ...
  await Promise.all(schemas.map(async schema => {
    ...
    if (!createValidator) {
      warn(`Skipping schema of unknown language ${type} in ${id} ${version}`)
      return
    }

    schema.validate = await createValidator({ url, value, cache })
      .catch(() => {
        warn(`Failed to create ${type} validator for ${id} ${version}`)
        return
      })
