...
import { asArray, addToArray, disallowSelect } from "./utils.js"
...
async function addValidators(format, service) {
  ...
  const versions = await Promise.all(
    ...
    .then(versions => Object.fromEntries(versions))

  if (Object.keys(versions).length) {
    if (!format.ValidateAll) {
      const defaultVersion = Object.keys(versions).sort().reverse()[0]
...
class ValidationService {
  ...
  // only used internally
  canEncode(encoding, format) {
    if (encoding.encodes && format.base) {
      const encodes = asArray(encoding.encodes)
      for (let base of asArray(format.base)) {
        if (encodes.indexOf(base) >= 0) {
          return true
        } else {
          return this.canEncode(encoding, this.getFormat(base))
        }
      }
    }
  }
...
const createService = async config => {
  ...
  for (const [uri, file] of Object.entries(schemaFiles)) {
    await cache.put(uri, fs.readFileSync(path.resolve(__dirname, file)), { valid: true })
  }
